angular.module('imageshare').controller('photosController', ['$rootScope','$scope','$http', function ($rootScope, $scope, $http) {
    ...
    $scope.editPhotoInformation = function(photo){
        var data = $.param({
            imageId: photo.imageId,
            caption: photo.imageCaption,
            imageDesc: photo.imageDesc,
        ...
        $http.post('http://imageshare.io/api/v1/editimage.php', data, config)
            .success(function (data) {
                if (data.error) {
                    console.log("Photos Edit: " + data.message);
                } else {
                    console.log("Successfully Edited " + photo.imageId);
                }
            })
            .error(function () {
